...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (walletTopUp && typeof walletTopUp === "number" && walletTopUp > 0) {
      const taxAmount = walletTopUp * 0.15;
      const netIncome = walletTopUp - taxAmount;
      user.taxBalance = (user.taxBalance || 0) + taxAmount;
      user.walletBalance += netIncome;
    ...
    return NextResponse.json<ApiResponse>(
      { 
        ...
        data: { name: user.name, walletBalance: user.walletBalance, taxBalance: user.taxBalance } 
      },
      { status: 200 }
    );
  } catch (error) {
